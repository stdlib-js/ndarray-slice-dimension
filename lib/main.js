...
function sliceDimension( x, dim, s ) {
	...
	if ( !isndarrayLike( x ) ) {
		throw new TypeError( format( '1kb4f', x ) );
	}
	if ( !isInteger( dim ) ) {
		throw new TypeError( format( '1kb7f', dim ) );
	}
	if ( isInteger( s ) || isSlice( s ) ) {
		...
		throw new TypeError( format( '1kbF9', s ) );
	}
	if ( arguments.length > 3 ) {
		options = arguments[ 3 ];
		if ( !isPlainObject( options ) ) {
			throw new TypeError( format( '1kb2V', options ) );
		}
		if ( hasOwnProp( options, 'strict' ) ) {
			if ( !isBoolean( options.strict ) ) {
				throw new TypeError( format( '1kb2o', 'strict', options.strict ) );
			}
